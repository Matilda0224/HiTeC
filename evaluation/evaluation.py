...
from HiTeC.paths import torch_load, missing_data_hint
...
def train_HE_predictor(X, edge_buckets, 
    ...
    # train set evaluation (eval mode)
...
def edge_prediction_eval(encoder, data, dataset,  num_splits=20, device='cuda:0'):
    ...
    e_path = osp.join(data.dataset_dir, 'edge_bucket_cns.pt')
    if not osp.exists(e_path):
        raise FileNotFoundError(
            missing_data_hint(data.name if hasattr(data, "name") else dataset)
            + f"\nMissing: {e_path}"
        )
    edge_buckets = torch_load(e_path)
