...
def drop_incidence_with_structure_score(hyperedge_index: torch.Tensor, drop_rate: float = 0.2) -> torch.Tensor:
    ...
    node_keep_score = 1.0 - node_deg / (node_deg.max() + 1e-6)
    edge_keep_score = 1.0 - edge_deg / (edge_deg.max() + 1e-6)

    structural_drop_prob = (node_keep_score + edge_keep_score) / 2
...
def sample_important_subhypergraph_swalk(
    ...
    """Build s-walk subgraphs for important nodes (global indices)."""
    device = x.device
    N = int(x.size(0))
...
def select_important_nodes(
    ...
    norm_degree = (node_degree - node_degree.min()) / (node_degree.max() - node_degree.min() + 1e-8)
