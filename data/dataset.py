...
from HiTeC.paths import DATA_DIR, torch_load, missing_data_hint
...
class BaseDataset(object):
    def __init__(self, name: str,  device: str):
        ...
        self.dataset_dir = str(DATA_DIR / self.name)
    ...
    def load_dataset(self):
        ...
        missing = [
            name for name in ('features.pt', 'hypergraph_dict.pt', 'labels.pt', 'texts.pt')
            if not osp.exists(osp.join(self.dataset_dir, name))
        ]
        if missing:
            raise FileNotFoundError(
                missing_data_hint(self.name)
                + f"\nMissing in {self.dataset_dir}: {', '.join(missing)}"
            )

        self.features = torch_load(osp.join(self.dataset_dir, 'features.pt'))
        self.hypergraph = torch_load(osp.join(self.dataset_dir, 'hypergraph_dict.pt'))
        self.labels = torch_load(osp.join(self.dataset_dir, 'labels.pt'))
        self.texts = torch_load(osp.join(self.dataset_dir, 'texts.pt'))

    def load_splits(self, seed: int):
        split_path = osp.join(self.split_dir, f'{seed}.pt')
        if not osp.exists(split_path):
            raise FileNotFoundError(f"Split file not found: {split_path}")
        return torch_load(split_path)
