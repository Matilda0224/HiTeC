...
class ContrastiveHardTextDataset(Dataset):
    ...
    def encode_texts(self, texts: list[str]) -> torch.Tensor:
        """Encode texts and aggregate embeddings (gradients enabled)."""
...
def contrastive_loss_with_hard_negative(z1, z2, temperature=0.5):
    """Contrastive loss with a hard negative per sample."""
    ...
    hard_logits = (z1 * hard_negatives).sum(dim=1, keepdim=True) / temperature
...
def contrastive_loss(z1, z2, temperature=0.5):
    """Standard contrastive loss between two views."""
    z1 = F.normalize(z1, dim=1)
    z2 = F.normalize(z2, dim=1)
...
def infonce_with_hard_neg(anchor, positive, hard_negative, temperature=0.2):
    ...
    logits_pos = torch.matmul(anchor, positive.T)
    logits_hard = torch.sum(anchor * hard_negative, dim=-1, keepdim=True)

    logits = torch.cat([logits_pos, logits_hard], dim=1)
...
def train_textencoder_hard_neg(encoder, dataloader, epochs=5, lr=2e-5, temperature=0.5):
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            # loss = _loss(raw_emb, aug_emb, nei_emb,temperature, True)
            # loss = loss()
            # break
            optimizer.zero_grad()
            loss.backward()
...
def encode_and_save(texts, text_encoder, batch_size=32, device="cuda"):
    """Encode texts with a trained TextEncoder."""
...
def build_all_prompts(
    ...
    """Build structure-aware prompts for every node."""
    all_prompts = []
...
def save_text_encoder(model, save_dir: str):
    """Save TextEncoder (encoder, projector, tokenizer)."""
...
def load_text_encoder(model_class, load_dir: str, device: str = "cuda"):
    """Load a saved TextEncoder checkpoint."""
...
if __name__ == '__main__':
    ...
    # train_textencoder(model, dataloader,epochs=5, lr=2e-5, temperature=0.5)
    # train_textencoder_hard_neg(model, dataloader, epochs=5, lr=2e-5, temperature=0.5)
    save_model_path = 'checkpoints/textencoder/pubmed/hard'
    # save_text_encoder(model, save_model_path)
    model = load_text_encoder(model, save_model_path)
    # PLM
    ...
    node_emb = encode_and_save(prompts, text_encoder=model, device=device)
